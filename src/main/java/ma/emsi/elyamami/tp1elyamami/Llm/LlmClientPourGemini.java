...
public class LlmClientPourGemini implements Serializable {
    ...
    public LlmClientPourGemini() {
         ...
         key = System.getenv("GEMINI_KEY");// URL endpoint avec la clé en paramètreString url =
        String url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=" + key;// Création du client REST
        Client clientRest = ClientBuilder.newClient();// Prépare l’accès à la ressource REST avec l’URLWebTarget target = clientRest.target(url);// Le format JSON sera utiliséInvocation.Builder requete = target.request(MediaType.APPLICATION_JSON_TYPE);
